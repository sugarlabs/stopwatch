...
class UnorderedHandler(dbus.gi_service.ExportedGObject):
    ...
    def set_tube(self, tube, is_initiator):
        ...
        # self.tube.add_signal_receiver(self.members_changed,
        # signal_name="MembersChanged",
        # dbus_interface="org.freedesktop.Telepathy.Channel.Interface.Group")
...
class HighScore:
    ...
    def __init__(
     ...
     score_translator=empty_translator,
     break_ties=False):
    ...
    def receive_message(self, message):
        ...
        if len(message) == 2:  # Remote has break_ties=False
            self._set_value_from_net(
                self._val_trans(message[0],
                                False),
                self._score_trans(message[1],
                                  False),
                None)
        elif len(message) == 3:
            self._set_value_from_net(
...
class AddOnlySet:
    ...
    def __init__(self, handler, initset=(), translator=empty_translator):
        ...
        self._listeners = []
            #This must be done before registering with the handler
    ...
    def _send(self, els):
        if len(els) > 0:
            self._handler.send(
                dbus.Array([self._trans(el, True) for el in els]))
...
class CausalDict:
    ...
    def __init__(
        ...
        handler,
     initdict=(),
     key_translator=empty_translator,
     value_translator=empty_translator):
        self._handler = handler
        self._dict = dict(initdict)
        self._clear = self._handler.get_index()
                                              # this must happen before
                                              # index_dict initialization, so
                                              # that self._clear is less than
                                              # any index in index_dict
        self._index_dict = dict(
            ((k, self._handler.get_index()) for k in initdict))
    ...
    def __delitem__(self, key):
        """Same as for dict"""
        del self._dict[key]
        n = self._handler.send(
            ((dbus.Int32(CausalDict.DELETE), self._key_trans(key, True))))
    ...
    def __setitem__(self, key, value):
        ...
        n = self._handler.send(
            dbus.Array([(dbus.Int32(CausalDict.ADD), self._key_trans(key, True), self._val_trans(value, True))]))
        self._index_dict[key] = n
    ...
    def pop(self, key, x=None):
        ...
        if t:
            n = self._handler.send(
                dbus.Array([(dbus.Int32(CausalDict.DELETE), self._key_trans(key, True))]))
            self._index_dict[key] = n
    ...
    def update(*args, **kargs):
        ...
        for p in list(d.items()):
            if (p[0] not in self._dict) or (self._dict[p[0]] != p[1]):
                ...
                self._dict[p[0]] = p[1]
        n = self._handler.send(
            dbus.Array([(dbus.Int32(CausalDict.ADD), self._key_trans(p[0], True), self._val_trans(p[1], True)) for p in newpairs]))
    ...
    def receive_message(self, msg, n):
        if n > self._clear:
            ...
            for m in msg:
                flag = int(
                    m[0])  # don't know length of m without checking flag
    ...
    def get_history(self):
        ...
        d = dbus.Array([(self._key_trans(p[0], True), self._val_trans(p[1], True))
                       for p in list(self._dict.items())])
        i = dbus.Array([(self._key_trans(p[0], True), self._handler.index_trans(p[1], True))
                       for p in list(self._index_dict.items())])
    ...
    def add_history(self, hist):
        ...
        d = dict(((self._key_trans(p[0], False), self._val_trans(p[1], False))
                 for p in hist[1]))
        i = [(self._key_trans(p[0], False), self._handler.index_trans(p[1], False))
             for p in hist[2]]
...
class UserDict(dbus.gi_service.ExportedGObject):
    ...
    def set_tube(self, tube, is_initiator):
        ...
        self.tube.add_signal_receiver(
            self.receive_value,
            signal_name='send_value',
            dbus_interface=UserDict.IFACE,
            sender_keyword='sender',
            path=self.PATH)
        self.tube.add_signal_receiver(
            self.tell_value,
            signal_name='ask_values',
            dbus_interface=UserDict.IFACE,
            sender_keyword='sender',
            path=self.PATH)
        self.tube.watch_participants(self.members_changed)
